...
/**
 * The Especially class represents a regular expression pattern used to match
 ...
 */
public class Especially extends RelationsCollection{
    private String regex;

    /**
     * Instantiates a new Especially.
     */
    public Especially() {
        this.regex = "<np>([^<]+)</np>\\s?(,?\\s?)especially\\s<np>([^<]+)</np>" +
                "(\\s?(,\\s?)<np>([^<]+)</np>)*"
                + "(\\s?,?\\s?(and|or)\\s<np>([^<]+)</np>)?";
    }
    @Override
